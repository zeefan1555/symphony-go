...
- smoke 运行日志：`go/.symphony/logs/run-*.jsonl`
...
- 原始运行日志：`go/.symphony/logs/run-*.jsonl` 和同名 `.human.log`
...
| `go/.symphony/logs/run-*.jsonl` | 机器可解析原始运行证据 | 运行自动生成，记录路径 |
| `go/.symphony/logs/run-*.human.log` | 人工可读时间线 | 运行自动生成，记录路径 |
...
```bash
latest=$(ls -t go/.symphony/logs/run-*.jsonl | head -n 1)
...
5. 确认 `go/.symphony/logs/run-*.jsonl` 和 `.human.log` 路径已经写进记录。
