...
- 旧顶层 `internal/orchestrator`、`internal/workspace`、`internal/codex`、`internal/workflow` 只允许作为迁移期遗留或兼容 shim；新增业务逻辑必须落到上述长期边界。旧 `internal/generated`、`internal/types`、`internal/config`、`internal/logging`、`internal/observability`、`internal/issuetracker` 与 `internal/control/hertz*` 已删除，不得重新引入。
