...
rm -rf "$repo_root/biz/handler" "$repo_root/biz/model" "$repo_root/biz/router"
cp -R "$tmp_dir/biz/handler" "$repo_root/biz/handler"
cp -R "$tmp_dir/biz/model" "$repo_root/biz/model"
cp -R "$tmp_dir/biz/router" "$repo_root/biz/router"

if [ -f "$repo_root/biz/router/register.go" ]; then
  perl -0pi -e 's/\n\tcontrol_http "github\.com\/zeefan1555\/symphony-go\/biz\/router\/control\/http"\n/\n/' "$repo_root/biz/router/register.go"
  perl -0pi -e 's/\n\tcontrol_http\.Register\(r\)\n/\n/' "$repo_root/biz/router/register.go"
else
  cat > "$repo_root/biz/router/register.go" <<'REGISTER_EOF'
