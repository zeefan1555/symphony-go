...
- review 通过后，不进入 PR 或 Merging 流程；框架将 issue 推进到 `Pushing`，并下发独立的 `Pushing` continuation prompt，再由同一个 agent session 推送当前目标分支、更新 workpad push evidence，并用 `Push: PASS` 交给框架移动到 `Done`。
...
11. 只有满足完成门槛后，当前 issue agent 才能移动 issue 到 `AI Review`，然后必须结束当前 turn，等待框架下发 `AI Review` continuation prompt。
    ...
    - 移动到 `AI Review` 的同一个实现 turn 不要输出 `Review: PASS` 或 `Push: PASS`，不要执行 AI Review 或 push。
...
3. 如果 review 通过，最终回复以 `Review: PASS` 开头；框架会把 issue 推进到 `Pushing`，并下发下一轮 `Pushing` continuation prompt。
4. 只有框架下发 `Pushing` continuation prompt 后，当前 issue agent 才执行 push：确认 repo root 仍在目标分支且 `git status --short` 干净。
