...
- review 通过后，不进入 PR 或 Merging 流程；直接推送当前目标分支到远端，然后更新 workpad push evidence，并把 issue 移动到 `Done`。
...
- `AI Review`：本地 commit 和验证已完成，同一个 issue agent 继续审核；通过后 push 目标分支并进入 `Done`，失败时进入 `Rework`。
...
3. 按状态进入对应流程：
   ...
   - `AI Review`：同一个 issue agent 继续审查；通过后 push 并移动到 `Done`，失败时进入 `Rework`。
...
11. 只有满足完成门槛后，当前 issue agent 才能移动 issue 到 `AI Review`，并继续同一个 session 的 review 流程。
    - 当前 issue agent 不直接从实现阶段跳到 `Done`；`Done` 只在 AI Review 通过且 push 成功后进入。
...
3. 如果 review 通过，确认 repo root 仍在目标分支且 `git status --short` 干净。
4. push 前再次确认本地 commit range、validation evidence 和 workpad 都已记录。
5. 执行 `git push origin <target>`，只推送配置的目标分支。
6. push 成功后，更新 workpad push evidence：
   - pushed branch
   - pushed `HEAD` short SHA
   - validation summary
   - AI Review verdict
7. 使用 `linear_graphql` 将 issue 移动到 `Done`。
8. 最终回复以 `Push: PASS` 开头，包含目标分支、pushed commit 和验证摘要。
...
- 在 `AI Review` 通过前不要 push；push 后必须更新 workpad 并移动 issue 到 `Done`。
