...
只有进入 `Merging` 阶段后才打开并遵守 `.codex/skills/pr/SKILL.md`；不要在实现或 AI Review 阶段创建 PR、合并 PR 或运行 `.codex/skills/pr/scripts/pr_merge_flow.sh`。

## Review 路由原则

- 默认路径是 `Todo -> In Progress -> AI Review -> Merging -> Done`。
- 单个 issue 默认只由同一个 agent session 端到端处理，不为 `AI Review` 额外启动第二个 agent。
- agent 可以多 turn、多 commit；完成 acceptance、validation、workpad 和本地 commit 后，移动到 `AI Review`，然后结束当前 turn，等待框架下发 `AI Review` continuation prompt。
- AI Review 只审本地结果：代码 diff、commit range、workpad、acceptance criteria 和 validation evidence。
- review 通过后，框架将 issue 推进到 `Merging`，并下发独立的 `Merging` continuation prompt，再由同一个 agent session 走 PR skill 快路径、更新 workpad merge evidence，并用 `Merge: PASS` 交给框架移动到 `Done`。
...
- `AI Review`：本地 commit 和验证已完成，同一个 issue agent 继续审核；通过后由框架进入 `Merging`，失败时进入 `Rework`。
- `Merging`：AI Review 已通过；同一个 issue agent 走 PR skill 快路径、写 merge evidence，并以 `Merge: PASS` 结束，随后框架进入 `Done`。
- `Pushing`：保留给不走 PR 的直接推送收口场景；不是默认路径。若 issue 已处于 `Pushing`，同一个 issue agent 推送目标分支、写 push evidence，并以 `Push: PASS` 结束，随后框架进入 `Done`。
...
3. 按状态进入对应流程：
   ...
   - `Merging`：同一个 issue agent 走 PR skill 快路径；成功后以 `Merge: PASS` 结束，由框架移动到 `Done`。
   - `Pushing`：保留兼容路径；只有 issue 已经处于该状态时才执行直接 push，成功后以 `Push: PASS` 结束，由框架移动到 `Done`。
...
9. 更新 workpad comment 的最终 checklist 和 validation notes。
   ...
   - 实现阶段不要把 PR URL 写进 workpad comment；PR 和 merge evidence 只在 `Merging` 阶段记录。
...
11. 只有满足完成门槛后，当前 issue agent 才能移动 issue 到 `AI Review`，然后必须结束当前 turn，等待框架下发 `AI Review` continuation prompt。
    - 当前 issue agent 不直接从实现阶段跳到 `Merging` 或 `Done`；`Merging` 只在 AI Review 通过后进入。
    - 移动到 `AI Review` 的同一个实现 turn 不要输出 `Review: PASS` 或 `Merge: PASS`，不要执行 AI Review 或 merge。
...
1. 只有框架下发 `AI Review` continuation prompt 后，当前 issue agent 才执行 AI Review：审查 issue、workpad、本地 diff、commit range 和验证证据；不要创建 PR，也不要运行 PR feedback sweep。
2. 如果发现需要修改的问题，当前 issue agent 将 findings 写入 workpad，移动 issue 到 `Rework`，然后基于发现继续修复或等待下一轮同 issue continuation。
3. 如果 review 通过，最终回复以 `Review: PASS` 开头；框架会把 issue 推进到 `Merging`，并下发下一轮 `Merging` continuation prompt。
4. 只有框架下发 `Merging` continuation prompt 后，当前 issue agent 才打开并遵守 `.codex/skills/pr/SKILL.md`。
5. merge 前再次确认本地 commit range、validation evidence 和 workpad 都已记录。
6. 使用 PR skill 快路径：确认 `pr_merge_flow.sh` 可执行，准备 PR title/body，运行脚本，等待 checks，并完成 squash merge / repo-root sync。
7. merge 成功后，更新 workpad merge evidence：
   - PR URL
   - merge commit
   - root status
   - validation summary
   - AI Review verdict
8. 最终回复以 `Merge: PASS` 开头，包含 PR URL、merge commit、root status 和验证摘要；框架会把 issue 移动到 `Done`。
...
- PR 已创建或更新并链接到 issue。
- PR checks 绿色。
- PR 已完成 merge，repo root 已同步到目标分支。
- workpad 已记录 merge evidence、merge commit、root status 和 validation summary。

## Blocked-access escape hatch（必须遵守）

仅当缺少必要工具、auth 或权限，且本 session 无法解决时使用。

- PR 或 merge 默认不是 valid blocker；必须先尝试可逆 fallback，例如 `git fetch`、确认 remote、确认当前 auth 状态、重试一次 PR skill 快路径。
...
- 如果缺少必要工具或 auth，移动 ticket 到 `Human Review`，并在 workpad 写入短 blocker brief，包含：
  - 缺少什么
  - 为什么阻塞必要 acceptance、validation 或 merge
  - 需要人类执行的精确解锁动作
...
- 不要创建 issue worktree、scratch checkout 或临时 clone。
- 只有 `Merging` 阶段可以打开或执行 `pr` skill、调用 `gh pr create`、`gh pr merge` 或 `pr_merge_flow.sh`。
...
- 未达到 `AI Review` 完成门槛前，不要移动到 `AI Review`；达到门槛后由当前 issue agent 移动到 `AI Review`，然后结束当前 turn，等待框架继续后续阶段。
- 不要把 `Human Review` 当成默认审核阶段；它只用于真实外部 blocker。
- 在 `AI Review` 中只做 review；实现修改应通过 `Rework` 返回实现阶段。
- 在 `Merging` 前不要创建或合并 PR；merge 后必须更新 workpad，并以 `Merge: PASS` 让框架收口 `Done`。
