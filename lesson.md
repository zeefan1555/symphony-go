...
## 2026-05-13: 代码证据默认给本地可点击链接

### 用户纠正

- 用户指出：提供代码证据时只给 `file:line` 不够方便，希望能给可跳转本地文件的链接，直接打开对应代码行。

### 错误模式

- 这是沟通错误：我把 SigNoz 字段或纯文本 `file:line` 当成足够证据，但没有考虑用户实际查看本地代码的操作成本。
- 这是交付格式错误：证据链虽然有行号，但没有按当前客户端支持的 Markdown 本地链接格式输出。

### 防复犯规则

- 最终回复里引用本机代码证据时，优先使用 Markdown 本地链接：`[path.go:123](/abs/path/path.go:123)`。
- SigNoz / OTel 字段中仍保留稳定文本 `evidence_locations`，但面向用户的汇报要把关键证据转成本地可点击链接。
- 多个仓库并行时，链接必须使用真实绝对路径，避免只写相对路径让用户二次搜索。

### 固定动作

- 汇报代码锚点前确认目标仓库根路径，并把关键 `file:line` 转成绝对路径链接：
