...
const (
	...
	AIReviewContinuationPromptText = "Continue in the same issue session and execute only the AI Review protocol for this issue. Re-check the issue, workpad, diff, commits, review feedback, and validation evidence. Do not merge, do not push, and do not move Linear to Merging or Done. If the work is correct, final reply must start with Review: PASS so the orchestrator can move the issue to Merging and start a separate Merging continuation turn; if it is not correct, record actionable findings, move the issue to Rework, and report concrete blockers."
