...
const (
	...
	AIReviewContinuationPromptText = "Continue in the same issue session and execute only the AI Review protocol for this issue. Re-check the issue, workpad, diff, commits, review feedback, and validation evidence. Do not push, do not run git push, and do not move Linear to Pushing or Done. If the work is correct, final reply must start with Review: PASS so the orchestrator can move the issue to Pushing and start a separate Pushing continuation turn; if it is not correct, record actionable findings, move the issue to Rework, and report concrete blockers."
	PushingContinuationPromptText  = "Continue in the same issue session and execute only the Pushing protocol for this issue. Confirm the configured target branch, confirm git status is clean, push the target branch with git push origin <target>, update the workpad once with push evidence, and report Push: PASS. Do not perform AI Review work and do not move Linear to Done from the agent; final reply must start with Push: PASS and include target branch, pushed commit, validation summary, and push result so the orchestrator can mark Done."
...
const (
	...
	StageContinuingPushing        RunStage = "continuing_pushing"
