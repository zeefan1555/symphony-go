...
`idl/orchestrator.thrift` 描述 issue run 调度、运行状态投影和控制入口的能力边界。它只表达 orchestrator 对外可被调用的稳定能力，不描述 running map、goroutine 或 retry queue 的实现细节。

`idl/workspace.thrift` 描述 workspace 路径解析、校验、准备和清理能力。路径安全、hook 执行和文件系统副作用仍由手写 workspace manager 拥有。

`idl/codex_session.thrift` 描述 Codex session 和 turn 生命周期能力。它不暴露 Go callback、函数类型或 app-server wire protocol 细节。
