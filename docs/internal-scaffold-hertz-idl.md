...
本文档记录旧内部架构脚手架 IDL 的迁移期契约。它描述内部子系统能力边界；当某个能力被明确纳入产品控制面 API 时，外部 HTTP 路由仍必须通过 `idl/main.proto` 和平铺领域 IDL 注册，而不是直接暴露 scaffold service。
...
`idl/tracker.proto`、`idl/runtime.proto` 和 `idl/observability.proto` 描述第一批补齐的产品控制面核心能力。tracker 只通过手写 issue tracker 边界读写 issue 状态；runtime 只暴露脱敏运行设置；observability 只暴露稳定运行快照投影。
...
该入口先运行 `buf lint`，再以 `idl/main.proto` 为唯一 service 来源，并通过根目录平铺领域 IDL 生成 `gen/hertz/model`、`gen/hertz/handler` 和 `gen/hertz/router`。旧 scaffold 生成命令已经删除；新增或修改产品控制面接口时，先改根目录 IDL，再更新手写 service / transport 绑定。
...
控制面 IDL 现在由根目录 `idl/` 的 `main.proto` 与平铺领域 IDL 组成，用来承载稳定产品控制面 API。`idl/main.proto` 是唯一 service 和所有业务 POST 接口 route annotations 的来源；平铺领域 IDL 只定义专属 Req/Resp 和嵌套模型。内部架构脚手架 IDL 不允许出现 `(api.get)`、`(api.post)` 或 `(api.path)` 这类 HTTP route annotations。
