...
Repository policy stays in `workflows/WORKFLOW-symphony-go.md`. Ticket editing rules, PR handling, validation gates, workpad shape, and handoff wording belong in the workflow prompt and agent skills unless a SPEC-required orchestrator gate needs a narrow state transition.
...
Symphony Go is not a general-purpose workflow engine or distributed job scheduler. Workflow branching, product-specific ticket edits, PR handling, and human handoff rules stay in `workflows/WORKFLOW-symphony-go.md`, agent skills, and narrow repo-local extensions instead of becoming a generic orchestration DSL.
