...
- `pull`：在实现和 push 前同步最新 `origin/main` 并处理冲突。
- `land`：当 issue 进入 `Merging` 时，打开并遵守 `.codex/skills/land/SKILL.md`，循环等待 review/checks、处理反馈并 squash merge。
...
- agent 可以多 turn、多 commit；完成 acceptance、validation、workpad、PR 创建/更新和 PR checks 后，移动到 `AI Review`，并在同一个 session 中继续审查 issue、workpad、PR、review feedback 和验证证据。
- review 通过后移动到 `Merging`；`Merging` 阶段仍在同一个 session 中使用 `land` skill 完成 squash merge。
...
- `Merging` 阶段走 `land` skill；不要直接调用 `gh pr merge`，也不要绕过 review/checks watcher。
...
- `In Progress`：正在实现。
- `AI Review`：PR 已附加、验证已完成，同一个 issue agent 继续审核；通过后进入 `Merging`，失败时进入 `Rework`。
- `Human Review`：仅用于真实外部 blocker 的人工 hold；不是默认 review 终点，也不主动轮询。
- `Merging`：AI Review 已通过；在 issue worktree 中打开并遵守 `.codex/skills/land/SKILL.md`，循环处理 review/checks 并 squash merge。
...
3. 按状态进入对应流程：
   ...
   - `Merging`：进入后打开并遵守 `.codex/skills/land/SKILL.md`；不要直接调用 `gh pr merge`。
...
当 ticket 已经有附加 PR，移动到 `AI Review` 前必须先跑完整 PR feedback sweep：
...
- GitHub push/PR 默认不是 valid blocker。必须先尝试 `push` / `pull` / `land` skill 中的 fallback，例如 alternate remote/auth mode、fetch、重新 push、检查 PR mergeability，然后继续 publish/review/land flow。
...
7. 每次 `git push` 前运行当前 scope 必需的 validation；如果失败，修复后重跑，直到绿色。
...
9. 使用 `push` skill push branch，创建或更新 PR，并把 PR URL 作为 attachment/link 关联到 Linear issue。
   - 确保 GitHub PR 有 `symphony` label。
10. merge 最新 `origin/main` 到 branch，解决冲突后重跑 checks。
11. 更新 workpad comment 的最终 checklist 和 validation notes。
    - 勾选已完成的 plan、acceptance、validation 项。
    - 在同一个 workpad comment 中加入最终 handoff notes，包括 commit 和 validation summary。
    - 不要把 PR URL 写进 workpad comment；PR 链接应保存在 issue attachment/link 上。
    - 如果执行中有任何不清楚的地方，在底部添加简短 `### Confusions` section。
    - 不要额外发布 completion summary comment。
12. 移动到 `AI Review` 前，poll PR feedback 和 checks：
    - 读取 PR Manual QA Plan comment（如果存在），并用它强化当前变更的 runtime/UI 验证。
    - 运行完整 PR feedback sweep protocol。
    - 确认最新 push 后 PR checks 全绿。
    - 确认 ticket 提供的全部 validation/test-plan 项已经在 workpad 中显式完成。
    - 重复 check-address-verify，直到没有 outstanding comments 且 checks 全绿。
    - 状态切换前重新打开并刷新 workpad，让 `Plan`、`Acceptance Criteria`、`Validation` 与已完成工作完全一致。
13. 只有满足完成门槛后，当前 issue agent 才能移动 issue 到 `AI Review`，并继续同一个 session 的 review 流程。
...
1. 当 issue 处于 `AI Review`，同一个 issue agent 审查 issue、workpad、PR、diff、commit range、review feedback 和验证证据。
...
4. 当 issue 处于 `Merging`，打开并遵守 `.codex/skills/land/SKILL.md`，循环执行 `land` skill，直到 PR merge 完成。不要直接调用 `gh pr merge`。
...
- PR feedback sweep 已完成，且没有 outstanding actionable comments。
...
- 在 `Merging` 中不要直接调用 `gh pr merge`；只允许执行 `land` skill。
