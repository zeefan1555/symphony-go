...
- `commit`：实现阶段产出清晰、逻辑完整的 commit。
- `pull`：在实现和进入 `Merging` 前同步最新 `origin/main` 并处理冲突。
- `pr`：当 issue 进入 `Merging` 时，打开并遵守 `.codex/skills/pr/SKILL.md`；该 skill 负责 push、创建或更新 PR、等待 checks、处理 PR readiness、squash merge，并报告 repo-root 状态。issue worktree agent 不负责写 repo-root `main` checkout。

## Review 路由原则

- `agent.review_policy.mode` 是 AI Review 路由的语义开关；本 workflow 使用 `AI Review` 作为示例 workflow 里 `Human Review` 的自动审核位置。
- 默认路径是 `In Progress -> AI Review -> Merging -> Done`。
- 单个 issue 默认只由同一个 agent session 端到端处理，不为 `AI Review` 或 `Merging` 额外启动第二个 agent。
...
- review 通过后移动到 `Merging`；`Merging` 阶段仍在同一个 session 中直接使用 `pr` skill 完成 push、PR、checks 和 squash merge。repo-root `main` checkout sync 由 orchestrator/operator 在 repo-root context 收尾，不由 issue worktree agent 执行。
- `Merging` 脚本成功后，agent 必须先更新同一个 workpad 的 merge evidence，再用 `Merge: PASS` 开头的最终回复报告 PR URL、merge commit 和 root status；Linear `Done` 状态由 orchestrator 收口，agent 不直接移动到 `Done`。
- review 不通过时移动到 `Rework`，同一个 issue agent 必须基于 review 发现重新计划、修复、验证和推送。
...
- `Merging` 阶段走 `pr` skill 快路径；不要直接调用 `gh pr merge`，也不要在脚本前重新展开实现、review 或完整历史 workpad。
...
- `In Progress`：正在实现。
- `AI Review`：本地 commit 和验证已完成，同一个 issue agent 继续审核；通过后进入 `Merging`，失败时进入 `Rework`。
- `Human Review`：仅用于真实外部 blocker 的人工 hold；不是默认 review 终点，也不主动轮询。
- `Merging`：AI Review 已通过；在 issue worktree 中打开并遵守 `.codex/skills/pr/SKILL.md`，优先运行 `.codex/skills/pr/scripts/pr_merge_flow.sh` 完成 PR merge flow。
- `Rework`：reviewer 要求修改；需要按 review 发现重新计划、实现、验证和推送。
- `Done`：终态；无需继续操作。
...
3. 按状态进入对应流程：
   ...
   - `AI Review`：同一个 issue agent 继续审查；通过后进入 `Merging`，失败时进入 `Rework`。
   - `Human Review`：只等待真实外部 blocker 的人工解锁；不要自行改代码或合并。
   - `Merging`：进入后打开并遵守 `.codex/skills/pr/SKILL.md`；不要直接调用 `gh pr merge`。
...
9. 运行 `pull` skill 同步最新 `origin/main`，然后把 pull/sync 结果记录到 workpad 的 `Notes`：
   - merge source(s)
   - 结果：`clean` 或 `conflicts resolved`
   - 同步后的 `HEAD` short SHA
10. compact context，然后进入执行。

## PR feedback sweep protocol（由 PR skill 执行）

当 ticket 进入 `Merging` 并由 `pr` skill 创建或更新 PR 后，必须在 `pr` skill/script 内完成 PR feedback sweep；不要在移动到 `AI Review` 前提前创建 PR 或重复 sweep：

1. 从 issue links/attachments 中识别 PR number。
2. 收集所有反馈渠道：
   - top-level PR comments：`gh pr view --comments`
   - inline review comments：`gh api repos/<owner>/<repo>/pulls/<pr>/comments`
   - review summaries/states：`gh pr view --json reviews`
3. 每一条 actionable reviewer comment（human 或 bot，包括 inline review comment）都视为 blocking，直到：
   - code/test/docs 已更新解决；或
   - 已在线程里给出明确、有理由的 pushback 回复。
4. 将每个 feedback item 及处理状态写进 workpad plan/checklist。
5. 反馈驱动的修改后重新运行 validation 并 push。
6. 重复 sweep，直到没有 outstanding actionable comments。

## Blocked-access escape hatch（必须遵守）

仅当缺少必要工具、auth 或权限，且本 session 无法解决时使用。

- GitHub push/PR 默认不是 valid blocker。必须先尝试 `pr` / `pull` skill 中的 fallback，例如 alternate remote/auth mode、fetch、重新 push、检查 PR mergeability，然后继续 publish/review/merge flow。
...
8. 使用 `commit` skill 提交当前 issue worktree 分支：
...
9. 更新 workpad comment 的最终 checklist 和 validation notes。
   ...
   - 不要把 PR URL 写进 workpad comment；PR 创建、链接和 GitHub label 由 `Merging` 阶段的 `pr` skill 负责。
   - 如果执行中有任何不清楚的地方，在底部添加简短 `### Confusions` section。
   - 不要额外发布 completion summary comment。
   - 正常简单任务不要为每个小命令更新 workpad；实现阶段至少覆盖 bootstrap、验证/commit handoff，Merging 阶段集中补一次 merge evidence。
...
11. 只有满足完成门槛后，当前 issue agent 才能移动 issue 到 `AI Review`，并继续同一个 session 的 review 流程。
    - 当前 issue agent 不直接从实现阶段跳到 `Merging`；`Merging` 只在同 session review 通过后进入。
    - 例外：如果按 blocked-access escape hatch 被工具或 auth 阻塞，可以带 blocker brief 和明确解锁动作移动到 `Human Review`。

## Step 3：AI Review 与 merge handling

1. 当 issue 处于 `AI Review`，同一个 issue agent 审查 issue、workpad、本地 diff、commit range 和验证证据；不要为了 review 提前创建 PR 或重复跑 PR feedback sweep。
2. 如果发现需要修改的问题，当前 issue agent 将 findings 写入 workpad，移动 issue 到 `Rework`，然后基于发现继续修复或等待下一轮同 issue continuation。
3. 如果 review 通过，当前 issue agent 移动 issue 到 `Merging`。
4. 当 issue 处于 `Merging`，打开并遵守 `.codex/skills/pr/SKILL.md`，优先运行 `.codex/skills/pr/scripts/pr_merge_flow.sh`，直到 PR 创建/更新、checks、squash merge 完成且 origin/main 可见。不要直接调用 `gh pr merge`。
5. merge 完成后，更新 workpad merge evidence，并以 `Merge: PASS` 开头最终回复；由 orchestrator 将 issue 移动到 `Done`。

## Merging 快路径

`Merging` 表示 AI Review 已通过；不要重新执行实现流程、不要重新审查完整 issue 历史、不要在脚本前做多轮 Linear comment 更新。

进入 `Merging` 后只做以下最小前置检查：

1. 确认已经打开 `.codex/skills/pr/SKILL.md`，并用 `test -x .codex/skills/pr/scripts/pr_merge_flow.sh` 确认脚本可执行；正常路径不要 `sed`/展开读取完整脚本内容。
2. 确认 issue worktree 干净，且当前分支不是 `main`。
3. 根据当前 commit/diff 准备中文 PR title/body。
4. 立即运行 `.codex/skills/pr/scripts/pr_merge_flow.sh`；PR script 和远端 checks 是 `Merging` 阶段的质量门槛。
5. 脚本成功后只集中更新一次 workpad，记录 PR URL、merge commit、root status、root sync ownership 和必要验证。
   - PR metadata 必须完整，包括 `symphony` label。
6. 不要从 issue worktree agent 执行 `git -C /Users/bytedance/symphony-go pull --ff-only origin main`、`git restore`、`git switch main` 或删除 repo-root 文件；repo-root `main` checkout sync 由 orchestrator/operator 在 repo-root context 处理。
7. 只有 PR merge 已完成、merge commit 可查询、`origin/main` 已 fetch 到该 merge commit，才用 `Merge: PASS` 开头最终回复。最终回复必须包含 PR URL、merge commit、root status；agent 不直接移动 issue 到 `Done`。
8. 不要重新执行实现流程、不要重新审查完整 issue 历史、不要重复读取 workflow 正文；`Merging` 阶段只做 PR script 前置检查、脚本执行和脚本后证据回写。

## Step 4：Rework 处理

1. 把 `Rework` 当作完整方案重置，而不是增量补丁。
2. 重新阅读完整 issue body 和所有人工 comments；明确本次 attempt 要做出哪些不同处理。
3. 关闭与该 issue 绑定的现有 PR。
4. 删除 issue 上现有 `## Codex Workpad` comment。
5. 从最新 `origin/main` 创建新分支。
...
- 在 `Merging` 中不要直接调用 `gh pr merge`；只允许执行 `pr` skill/script。
- 在 `Merging` 中不要直接移动 issue 到 `Done`；脚本成功、workpad 更新后用 `Merge: PASS` 最终回复，等待 orchestrator 收口。
