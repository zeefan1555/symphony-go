...
5. merge 完成后，更新 workpad merge evidence，并移动 issue 到 `Done`。
...
1. 读取当前 issue、唯一 `## Codex Workpad`、`git status --short --branch`、`git rev-parse --short HEAD` 和当前分支名。
...
7. 只有 PR merge 已完成、merge commit 可查询、`origin/main` 已 fetch 到该 merge commit，才移动 issue 到 `Done`。
