...
- implementer agent 可以多 turn、多 commit；完成 acceptance、validation、workpad、PR 创建/更新和 PR checks 后，移动到 `AI Review`。
- `AI Review` 由 reviewer agent 审查 issue、workpad、PR、review feedback 和验证证据。
- reviewer 通过后移动到 `Merging`；`Merging` 阶段使用 `land` skill 完成 squash merge。
- reviewer 不通过时移动到 `Rework`，下一轮必须基于 review 发现重新计划、修复、验证和推送。
...
- `In Progress`：正在实现。
- `AI Review`：PR 已附加、验证已完成，等待 reviewer agent 审核；通过后进入 `Merging`，失败时进入 `Rework`。
...
13. 只有满足完成门槛后，implementer agent 才能移动 issue 到 `AI Review`。
...
1. 当 issue 处于 `AI Review`，reviewer agent 审查 issue、workpad、PR、diff、commit range、review feedback 和验证证据。
2. 如果发现需要修改的问题，reviewer agent 将 findings 写入 workpad，移动 issue 到 `Rework`，然后结束本轮。
